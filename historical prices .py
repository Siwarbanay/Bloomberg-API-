...
def field_description(
    ...
    while page < (MAX_PAGE if more_than_one_page else 2):
        ...
        if download:
            with open(
                f"schema vers data licence {datetime.today():%Y%m%d_%H%M%S}.json",
                "wb",
            ) as outputfile:
...
def history_request(tickers, fields, startDate, endDate, session=None):
    ...
    with session.get(output_url, stream=True) as response:
        LOG.info(f"Encoding is {response.headers['content-encoding']}")
        json_content = decode(response)
        df = pd.json_normalize(json_content)[["IDENTIFIER", "DATE", "PX_DIRTY_ASK"]].pivot(
            index="DATE", columns="IDENTIFIER", values=["PX_DIRTY_ASK"]
        )
...
tickers = pd.DataFrame(
    {
        "identifierType": ["ISIN","ISIN","ISIN","ISIN"],
        "identifierValue": [ "],
    }
)
fields = ["PX_DIRTY_ASK"]
...
data = history_request(
    tickers, fields, date(2024, 12, 31), date(2025, 4, 30)
     )
...
print(data)
data.to_csv(file_path, index=True, encoding='utf-8')
