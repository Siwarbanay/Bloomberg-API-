...
def history_request(tickers, fields, startDate, endDate, session=None):
    ...
    name = "GDAFAM" + str(uuid.uuid1())[:6]

    # build json of the request
    data = build_history_request(name, tickers, fields, startDate, endDate)
    ...
    with session.get(output_url, stream=True) as response:
        LOG.info(f"Encoding is {response.headers['content-encoding']}")
        json_content = decode(response)
        df = pd.json_normalize(json_content)[["IDENTIFIER", "DATE", "PX_LAST"]].pivot(
            index="DATE", columns="IDENTIFIER", values="PX_LAST"
        )
...
# data = data_request(
#     ["AAPL US Equity"], ["NAME", "SECURITY_TYP", "COUNTRY"]
# )  # open source fields
# print(data)
# data = history_request(
#     ["AAPL US Equity"], ["PX_LAST"], date(2024, 12, 10), date.today()
# )

# tickers = pd.DataFrame(
#     {
#         "identifierType": ["TICKER"],
#         "identifierValue": ["EURGBP1W BGN Curncy"],
#         "override_mnemonic": ["FWD_CURVE_QUOTE_FORMAT"],
#         "override_value": ["OUTRIGHT"],
#     }
# )

# data = data_request(tickers, ["PX_LAST"])

# field_metadata("PX_LAST")
